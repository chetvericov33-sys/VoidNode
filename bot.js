// ============================================================
// БОТ VOID NODE — ПОЛНАЯ РАБОЧАЯ ВЕРСИЯ 0.7
...
async function checkContractAdvanced(address) {
    ...
    try {
        ...
        if (lowerAddress.includes('dead') || lowerAddress.includes('0000') || lowerAddress.endsWith('f')) {
            ...
            result.warnings.push('🚫 Обнаружен подозрительный паттерн (honeypot)');
            result.riskScore += 40;
...
async function handleContractCheck(chatId, address, lang, messageId) {
    ...
    try {
        ...
        message += '🚫 *Honeypot:* ' + (result.isHoneypot ? 'Обнаружен!' : 'Не обнаружен') + '\n';
        message += '\n━━━━━━━━━━━━━━━━━━━━━━━\n';
...
async function handleCallback(update) {
    ...
    try {
        ...
        // HELP FAQ: normalize callback help_qN -> translation help_answer_qN.
        // This prevents the production bug `help_answer_help_q4`.
        if (/^help_q[1-9]$/.test(data)) {
            var helpQuestionKey = data.replace(/^help_/, '');
            var helpAnswerKey = 'help_answer_' + helpQuestionKey;
            var helpAnswer = getText(lang, helpAnswerKey);
            await sendMessage(chatId, formatHelpAnswer(helpAnswer, lang));
            return;
        }
