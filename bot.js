// ============================================================
// БОТ VOID NODE — READ-ONLY PRODUCTION VERSION 1.8.0
...
if (!ENCRYPTION_KEY || ENCRYPTION_KEY.length < 32) {
    console.error('❌ ENCRYPTION_KEY is required and must be at least 32 characters long');
    process.exit(1);
}
if (!ADMIN_CHAT_ID) {
...
class RedisStorage {
    ...
    async atomicIncrement(key, ttl) {
        if (ttl === undefined) ttl = null;
        try {
            if (!this.isRedisAvailable) {
                throw new Error('Redis unavailable; refusing non-atomic usage increment');
            }
            var count = await this.redis.incr(key);
            if (count === 1 && ttl) await this.redis.expire(key, ttl);
            return Number(count);
        } catch (error) {
...
async function sendTyping(chatId) {
    ...
    } catch (e) { console.error('Recoverable error:', e.message); }
}
    ...
    if (LIFETIME_VIP_USERS.includes(parseInt(chatId))) {
        return {
            ...
            limits: {
                ...
                diary: true, ranking: true, ai: Infinity, csv: true, kill_switch: true,
...
async function checkLimit(chatId, feature) {
    ...
    // Production limits must be atomic. Redis INCR prevents concurrent requests
    // from both observing the same counter and exceeding the daily limit.
    if (!VOID_KV.isRedisAvailable) {
        return { allowed: false, reason: '⚠️ Usage limits are temporarily unavailable. Please try again in a minute.' };
    }
    var key = 'usage_' + chatId + '_' + feature + '_' + new Date().toISOString().split('T')[0];
    try {
        var count = await VOID_KV.atomicIncrement(key, 86400);
        if (count > limit) {
            // Do not allow a race to consume an additional paid operation. The
            // counter is deliberately kept monotonic; requests over the limit fail.
            return { allowed: false, reason: '📊 Limit exceeded. Upgrade: /subscribe' };
        }
        return { allowed: true, remaining: Math.max(0, limit - count) };
    } catch (error) {
        console.error('checkLimit failed closed:', error.message);
        return { allowed: false, reason: '⚠️ Usage limits are temporarily unavailable. Please try again in a minute.' };
    }
}
...
var LANGUAGES = {
    ru: {
        ...
        help_q6: 'Работает ли Void Node в режиме торговли?',
        ...
        help_answer_q3: '🔐 *Зачем подключать биржу?*\n━━━━━━━━━━━━━━━━━━━━━━━\n\nПодключение биржи даёт доступ к ключевым функциям:\n\n1️⃣ Анализ портфеля — бот видит активы и даёт рекомендации\n2️⃣ Холодный душ — уведомления о резких изменениях риска\n3️⃣ Оповещения — уведомления по вашим активам\n4️⃣ История и отчёты — сохранение результатов анализа\n5️⃣ AI-советник — объяснение риска и данных портфеля\n\n🔒 Ключи используются только для чтения, шифруются и не нужны для выполнения сделок.',
        ...
        help_answer_q6: '🛡️ *Как работает торговля?*\n━━━━━━━━━━━━━━━━━━━━━━━\n\nVoid Node работает только в read-only режиме. Бот анализирует баланс и рыночные данные, но не может покупать, продавать, выставлять или отменять ордера. Это сделано, чтобы снизить операционный риск.',
        help_answer_q7: '❄️ *Что такое холодный душ?*\n━━━━━━━━━━━━━━━━━━━━━━━\n\nРежим наблюдения за резкими движениями портфеля:\n\n✅ Бот проверяет подключённые активы каждые 15 минут\n✅ При резком падении отправляет предупреждение\n✅ Показывает, какие активы изменились\n✅ Предлагает открыть анализ\n\n🛡️ Void Node не продаёт активы и не выполняет сделки.',
        ...
        plans_pro: '🚀 *PRO*\n💰 1 000 ₽ • 30 дней\n\n📋 *Что входит:*\n• 📊 30 анализов портфеля в день\n• 🛡️ 50 антискам-проверок\n• 🔔 15 оповещений\n• ❄️ *Защита от обвалов* — расширенные риск-уведомления\n• 🤖 AI-советник (20/день)\n• 📊 Полные отчёты + CSV\n• 🆘 Расширенный контроль риска\n\n💡 *Идеально для:* пользователей, которым нужен постоянный контроль портфеля.',
        plans_vip: '👑 *VIP*\n💰 1 500 ₽ • 30 дней\n\n📋 *Что входит:*\n• ✅ Все основные функции без лимитов\n• ❄️ *Защита от обвалов* — безлимитные риск-уведомления\n• 🤖 AI-советник — безлимит\n• 📊 Полные отчёты + CSV\n• 🆘 Расширенный контроль риска\n• ⚡ Приоритетная поддержка 24/7\n\n💡 *Идеально для:* пользователей, которым нужен полный контроль портфеля.',
        ...
        panic_start: '❄️ *Холодный душ активирован.*\n\nБуду отслеживать подключённые активы каждые 15 минут. При резком падении отправлю предупреждение и предложу открыть анализ. Void Node не совершает сделки.',
        panic_stop: '❄️ Холодный душ остановлен.',
        panic_trigger: '🚨 *ХОЛОДНЫЙ ДУШ СРАБОТАЛ!*\n\nОбнаружено падение >5% по нескольким активам.\n\n⚠️ Проверьте портфель и примите решение вручную. Void Node не совершает сделки.',
        panic_convert: '📊 Открыть анализ портфеля',
        panic_converted: '📊 Анализ открыт. Решение о любых операциях остаётся за пользователем.',
        about_title: 'ℹ️ *О БОТЕ*\n━━━━━━━━━━━━━━━━━━━━━━━',
        about_version: '📌 *Версия:* 0.7',
    ...
    en: {
        ...
        help_q6: 'Does Void Node trade for me?',
        ...
        help_answer_q3: '🔐 *Why connect exchange?*\n━━━━━━━━━━━━━━━━━━━━━━━\n\nConnecting your exchange gives you access to key features:\n\n1️⃣ Portfolio analysis — bot sees your assets and gives recommendations\n2️⃣ Panic alerts — notifications about sharp portfolio moves\n3️⃣ Alerts — notifications for your assets\n4️⃣ History and reports — saved analysis results\n5️⃣ AI advisor — explanations of portfolio risk and data\n\n🔒 Keys are used only for reading, encrypted, and never used to execute trades.',
        ...
        help_answer_q6: '🛡️ *Does Void Node trade for me?*\n━━━━━━━━━━━━━━━━━━━━━━━\n\nNo. Void Node works in read-only mode. It can analyze balances and market data, but it cannot buy, sell, place, or cancel orders. This reduces operational risk.',
        ...
        plans_pro: '🚀 *PRO*\n💰 1 000 ₽ • 30 days\n\n📋 *What\'s included:*\n• 📊 30 portfolio analyses per day\n• 🛡️ 50 anti-scam checks\n• 🔔 15 alerts\n• ❄️ *Crash protection* — expanded risk alerts\n• 🤖 AI advisor (20/day)\n• 📊 Full reports + CSV\n• 🆘 Expanded risk controls\n\n💡 *Perfect for:* users who want continuous portfolio monitoring.',
        plans_vip: '👑 *VIP*\n💰 1 500 ₽ • 30 days\n\n📋 *What\'s included:*\n• ✅ All core features without limits\n• ❄️ *Crash protection* — unlimited risk alerts\n• 🤖 AI advisor — unlimited\n• 📊 Full reports + CSV\n• 🆘 Expanded risk controls\n• ⚡ 24/7 priority support\n\n💡 *Perfect for:* users who want full portfolio oversight.',
        ...
        panic_start: '❄️ *Panic alerts activated.*\n\nI will monitor connected assets every 15 minutes. If a sharp drop is detected, I will send an alert and suggest opening analysis. Void Node never executes trades.',
        panic_stop: '❄️ Panic mode stopped.',
        panic_trigger: '🚨 *PANIC MODE TRIGGERED!*\n\nDetected drop >5% across multiple assets.\n\n⚠️ Review the portfolio and decide manually. Void Node never executes trades.',
        panic_convert: '📊 Open portfolio analysis',
        panic_converted: '📊 Analysis opened. Any trading decision remains with the user.',
...
async function loadUserKeys(chatId) {
    ...
    try {
        ...
        if (!decryptedApiKey || !decryptedSecretKey || parsed.exchangeId !== 'binance') return null;
        return {
...
async function checkContractAdvanced(address) {
    ...
    try {
        ...
        // Honeypot status is intentionally UNKNOWN here. An address-string heuristic
        // is not a honeypot detector and creates false positives. Use a real simulation
        // provider before asserting buy/sell capability.
        result.warnings.push('⚠️ Honeypot не проверен: для достоверного результата нужен симуляционный провайдер.');
        result.details.push('🔎 Honeypot: UNKNOWN');
        ...
        if (result.riskScore > 70) {
            ...
            result.recommendations.push('✅ Контракт выглядит безопасно');
        }
...
async function showAutotradeMenu(chatId) {
    ...
    await sendMessage(chatId, lang === 'ru'
        ? '🛡️ Void Node работает только в read-only режиме. Торговые операции отключены: бот не может покупать, продавать, выставлять или отменять ордера.'
        : '🛡️ Void Node operates in read-only mode. Trading is disabled: the bot cannot buy, sell, place, or cancel orders.');
}
...
async function handleContractCheck(chatId, address, lang, messageId) {
    ...
    try {
        ...
        message += '🚫 *Honeypot:* ' + (result.isHoneypot === null ? 'UNKNOWN — не проверен' : (result.isHoneypot ? 'Обнаружен!' : 'Не обнаружен')) + '\n';
        message += '\n━━━━━━━━━━━━━━━━━━━━━━━\n';
        ...
        await sendUpdatedMessage(chatId, '❌ ' + (lang === 'ru' ? 'Ошибка проверки контракта' : 'Contract check error') + ': ' + error.message, null, 'Markdown', messageId);
    }
}
...
async function handleWalletCheck(chatId, address, lang, messageId) {
    ...
    try {
        ...
        await sendUpdatedMessage(chatId, '❌ ' + (lang === 'ru' ? 'Ошибка проверки кошелька' : 'Wallet check error') + ': ' + error.message, null, 'Markdown', messageId);
    }
}
...
async function handleDEXCheck(chatId, address, lang, messageId) {
    ...
    try {
        ...
        await sendUpdatedMessage(chatId, '❌ ' + (lang === 'ru' ? 'Ошибка DEX проверки' : 'DEX check error') + ': ' + error.message, null, 'Markdown', messageId);
    }
}
...
class AIContext {
    ...
    getSystemPrompt(lang) {
        if (lang === 'ru') {
            return 'Ты — профессиональный крипто-советник Void Node. Ты ведешь ЕСТЕСТВЕННЫЙ ДИАЛОГ с пользователем.\n\n' +
                'ПРАВИЛА ОБЩЕНИЯ:\n' +
                '1. Отвечай как человек — не шаблонно, а в разговорном стиле.\n' +
                '2. Если пользователь пишет "привет" — ответь приветствием, представься и спроси, чем помочь.\n' +
                '3. Если пользователь задает вопрос — дай развернутый ответ с конкретными цифрами.\n' +
                '4. Используй данные из портфеля пользователя (если они есть).\n' +
                '5. Если данных нет — скажи честно и предложи выполнить /analyze.\n' +
                '6. Будь дружелюбным, но профессиональным.\n' +
                '7. Используй эмодзи для наглядности.\n' +
                '8. Поддерживай диалог — задавай уточняющие вопросы.\n\n' +
                'ТЫ НЕ ДОЛЖЕН:\n' +
                '- Давать советы "купи/продай"\n' +
                '- Предсказывать цену\n' +
                '- Придумывать данные\n' +
                '- Отвечать шаблонно ("ты написал привет, что мне с этим делать")\n\n' +
                'Помни: ты — умный и дружелюбный советник, который заботится о деньгах пользователя.';
        } else {
            return 'You are a professional crypto advisor "Void Node". You have a NATURAL CONVERSATION with the user.\n\n' +
                'RULES:\n' +
                '1. Respond like a human — not template, but conversational.\n' +
                '2. If user says "hello" — greet, introduce yourself and ask how to help.\n' +
                '3. If user asks a question — give a detailed answer with specific numbers.\n' +
                '4. Use portfolio data (if available).\n' +
                '5. If no data — say so honestly and suggest running /analyze.\n' +
                ...
                '- Predict price\n' +
                '- Make up data\n' +
                '- Reply with templates ("you said hello, what should I do with that")\n\n' +
                'Remember: you are a smart and friendly advisor who cares about the user\'s money.';
        }
    }
...
async function handleAICommand(chatId, question, lang, messageId) {
    ...
    var keyboard = {
        inline_keyboard: [
            [{ text: '💬 ' + (lang === 'ru' ? 'Продолжить диалог' : 'Continue dialogue'), callback_data: 'ai_chat_start' }],
            [{ text: getText(lang, 'back_to_functions'), callback_data: 'back_to_functions' }]
        ]
    };
...
async function handleCallback(update) {
    ...
    try {
        ...
        if (data === 'autotrade_level1' || data === 'autotrade_level2' || data === 'autotrade_level3' || data === 'autotrade_level4' || data === 'autotrade_stop') { await showAutotradeMenu(chatId); return; }
        ...
        if (data === 'panic_convert_all') { await showAutotradeMenu(chatId); return; }
        ...
        if (data === 'action_rebalance') {
            await sendMessage(chatId, lang === 'ru' ? '🔄 Ребаланс сейчас работает только как рекомендация: Void Node не выполняет сделки. Открой полный анализ, чтобы увидеть отклонения от целевых весов.' : '🔄 Rebalance is recommendation-only: Void Node does not execute trades. Open full analysis to review deviations from target weights.');
            return;
        }
    ...
    try {
        ...
        if (state === 'waiting_for_keys' || state === 'waiting_for_keys_vip') {
            ...
            if (parts.length === 2) {
                ...
                // API credentials are sensitive: remove the user's message from Telegram
                // immediately, before validation/storage.
                await deleteUserMessage(chatId, messageId);
                await sendTyping(chatId);
                await sendMessage(chatId, '🔍 Проверяю подключение…');

                try {
                    // Validate credentials before persisting anything. The bot only calls
                    // read-only CCXT methods; it never creates/cancels orders.
                    await connectExchange('binance', apiKey, secretKey);
                    ...
                    console.error('Exchange credential validation failed for ' + chatId + ':', validationError.message);
                    await setData('state_' + chatId, 'idle');
                    await sendMessage(chatId, getText(lang, 'connect_fail'));
                    return;
                }
                if (state === 'waiting_for_keys_vip') {
        ...
        if (cleanText === '/panic') { var panicPlan = await getUserPlan(chatId); if (!panicPlan.limits.panic) { await sendMessage(chatId, getText(lang, 'calendar_pro_only')); return; } await setData('panic_' + chatId, JSON.stringify({ active: true, lastCheck: Date.now() })); await sendMessage(chatId, getText(lang, 'panic_start')); return; }
...
async function handleCalendarCommand(chatId, lang, messageId) {
    ...
    try {
        ...
        await sendUpdatedMessage(chatId, '⚠️ Экономический календарь временно недоступен. Попробуйте позже.', null, 'Markdown', messageId);
    }
}
...
// Trading execution was intentionally removed in the read-only production build.
    ...
    for (var k = 0; k < keys.length; k++) {
        ...
        try {
            ...
            if (panicTriggered) {
                ...
                var keyboard = {
                    inline_keyboard: [
                        [{ text: '📊 Open analysis', callback_data: 'action_analyze' }],
...
// No panic conversion handler: Void Node never executes trades. Panic mode is alert-only.
