...
const LANGUAGES = {
    ru: {
        ...
        main_header: (name, mode, id, plan, expires) => 'Пользователь: ' + name + ' | ' + mode + ' | ID: ' + id + '\nТариф: ' + plan + ' (до ' + expires + ')',
        ...
        plans_already: (plan) => 'У вас уже активен ' + plan,
        ...
        greeting_morning: (name) => 'Доброе утро, ' + name + '!',
        greeting_afternoon: (name) => 'Добрый день, ' + name + '!',
    ...
    en: {
        ...
        main_header: (name, mode, id, plan, expires) => 'User: ' + name + ' | ' + mode + ' | ID: ' + id + '\nPlan: ' + plan + ' (until ' + expires + ')',
        ...
        plans_already: (plan) => 'You already have ' + plan,
        ...
        main_help_old: 'Help',
        greeting_morning: (name) => 'Good morning, ' + name + '!',
        greeting_afternoon: (name) => 'Good afternoon, ' + name + '!',
...
async function handleNewsCommand(chatId, coin, lang, messageId) {
    ...
    report += 'По твоим активам: ' + result.assets.map(a => a.symbol).join(', ') + '\n\n';
...
async function composeReport(engineResult, mode, lang, dailyChange = 0) {
    ...
    if (signals && signals.length > 0) {
        ...
        for (const s of signals) {
            const emoji = s.type === 'bullish' ? '' : s.type === 'bearish' ? '' : s.type === 'overbought' ? '' : s.type === 'oversold' ? '' : '';
