...
async function botDeleteMessage(chatId, messageId) {
  try {
    ...
    return response;
  } catch (error) { console.error('Delete message error:', error); return null; }
}
async function deleteUserMessage(chatId, messageId) {
  ...
  } catch (error) { console.error('Failed to delete user message:', error); }
}
...
async function sendUpdatedMessage(chatId, text, keyboard = null, parseMode = 'Markdown', userMessageId = null) {
  ...
  if (userMessageId) await deleteUserMessageWithDelay(chatId, userMessageId, 1500);
...
async function sendMessage(chatId, text, keyboard = null, parseMode = 'Markdown') {
  ...
  try {
    ...
    return response;
  } catch (error) { console.error('Send message error:', error); return null; }
}
...
// ============================================================
// 6. KV ФУНКЦИИ (сохранение/загрузка ключей, состояния, онбординг)
// ============================================================
...
function generateCSV(engineResult) {
  ...
  csv += `\nRisk Level,${engineResult.riskLevel}\n`;
  csv += `Risk Score,${engineResult.riskScore}\n`;
  csv += `Total USDT,${engineResult.totalUSDT.toFixed(2)}\n`;
  if (engineResult.btcMetrics && engineResult.btcMetrics.sharpe !== undefined) {
    ...
    csv += `Sortino Ratio,${engineResult.btcMetrics.sortino.toFixed(2)}\n`;
    csv += `VaR (95%),${engineResult.btcMetrics.var.toFixed(2)}\n`;
  }
  return csv;
}

// ============================================================
// 12. fetchWithRetry И measurePerformance
// ============================================================
...
async function analyzePortfolioEngine(exchangeId, apiKey, secretKey, chatId = null, lang = 'ru') {
      ...
      if (alt.price > 0) {
        ...
        if (sellAmount > 0.001) {
          recommendations.push({
            id: `reduce_${alt.asset}_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`,
            action: 'sell',
            asset: alt.asset,
  ...
  for (const alt of altcoins) {
    ...
    if (!WHITELIST_SYMBOLS.includes(symbol) && alt.volume && alt.volume < 100000) {
      ...
      if (alt.price > 0) {
        recommendations.push({
          id: `sell_junk_${alt.asset}_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`,
          action: 'sell',
          asset: alt.asset,
          symbol: symbol,
  ...
  for (const alt of altcoins) {
    if (alt.delisted) {
      ...
      if (alt.price > 0) {
        recommendations.push({
          id: `sell_delisted_${alt.asset}_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`,
          action: 'sell',
          asset: alt.asset,
          symbol: `${alt.asset}/USDT`,
          amount: alt.value / alt.price,
  ...
  if (altPercent > 40) {
    ...
    for (const alt of altcoins) {
      if (alt.price > 0) {
        ...
        if (sellAmount > 0.001) {
          recommendations.push({
            id: `reduce_alt_exposure_${alt.asset}_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`,
            action: 'sell',
            asset: alt.asset,
            symbol: `${alt.asset}/USDT`,
            amount: sellAmount,
  ...
  if (btcMetrics.ma200 && btcMetrics.ma200.ma > 0) {
    ...
    if (currentPrice < ma200) {
      ...
      if (usdtPercent < 30) {
        ...
        riskScore += 10;
        recommendations.push({
          id: `buy_stable_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`,
          action: 'buy',
          asset: 'USDT',
          symbol: 'USDT/USDT',
  ...
  for (const metric of assetMetrics) {
    ...
    if (metric.rsi && metric.rsi.signal === 'overbought') {
      ...
      recommendations.push({
        id: `sell_rsi_${metric.asset}_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`,
        action: 'sell',
        asset: metric.asset,
      ...
      recommendations.push({
        id: `buy_rsi_${metric.asset}_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`,
        action: 'buy',
        asset: metric.asset,
        symbol: `${metric.asset}/USDT`,
...
// ============================================================
// 20. ИСПОЛНЕНИЕ РЕКОМЕНДАЦИЙ (С ПРОВЕРКАМИ)
// ============================================================
...
// ============================================================
// 23. НОВОСТИ (АГРЕГАЦИЯ С ФИЛЬТРАЦИЕЙ)
// ============================================================
...
// ============================================================
// 26. ОПОВЕЩЕНИЯ (С ПРОВЕРКОЙ ДУБЛИКАТОВ)
// ============================================================
...
// ============================================================
// 27. ХОЛОДНЫЙ ДУШ (ПРОВЕРКА + КОНВЕРТАЦИЯ)
// ============================================================
...
// ============================================================
// 29. МЕНЮ ПОМОЩИ (С 10 КНОПКАМИ + МОДЕРАТОР)
// ============================================================
