...
// ============================================================
// 3. МУЛЬТИЯЗЫЧНОСТЬ (LANGUAGES) - ПОЛНЫЙ РУССКИЙ И АНГЛИЙСКИЙ
// ============================================================
const LANGUAGES = {
  ru: {
    // Общие
    ...
    // Новые ключи для дополнительных функций
    ...
    alert_delete_prompt: '📋 Выберите оповещение для удаления:',
    alert_triggered: '🔔 *Оповещение сработало!*\n\nСимвол: {symbol}\nУсловие: {condition}\nТекущее значение: {value}',
    ...
    panic_trigger: '🚨 *Холодный душ сработал!*\n\nBTC упал на {percent}% за последние 15 минут.\n\n⚠️ Рекомендуется конвертировать часть портфеля в стейблы.\nНажмите кнопку ниже, чтобы выполнить конвертацию.',
    panic_convert: '🔄 Конвертировать в стейблы',
    panic_converted: '✅ Конвертация выполнена. Портфель в безопасности.',
    diary_saved: '✅ Настроение сохранено.',
    diary_warning: '⚠️ Вы тревожны уже {days} дней подряд. Рекомендую сделать перерыв.',
    kill_switch_confirm: '⚠️ *ПОДТВЕРДИТЕ KILL SWITCH*\n\nЭто действие НЕОБРАТИМО! Все ордера будут отменены, ключи удалены, бот заблокирован.',
    kill_switch_activated: '🛑 *KILL SWITCH АКТИВИРОВАН*\n\n• Ордера отменены\n• Ключи удалены\n• Бот заблокирован\n\nДля разблокировки: /reset',
    kill_switch_cancelled: '❌ Kill Switch отменён.',
  },
  en: {
    ...
    alert_delete_prompt: '📋 Choose alert to delete:',
    ...
    panic_trigger: '🚨 *Panic mode triggered!*\n\nBTC dropped by {percent}% in the last 15 minutes.\n\n⚠️ It\'s recommended to convert part of portfolio to stables.\nPress the button below to convert.',
    panic_convert: '🔄 Convert to stables',
    panic_converted: '✅ Conversion completed. Portfolio is safe.',
    diary_saved: '✅ Mood saved.',
    diary_warning: '⚠️ You\'ve been anxious for {days} days in a row. I recommend taking a break.',
    kill_switch_confirm: '⚠️ *CONFIRM KILL SWITCH*\n\nThis action is IRREVERSIBLE! All orders will be cancelled, keys deleted, bot locked.',
    kill_switch_activated: '🛑 *KILL SWITCH ACTIVATED*\n\n• Orders cancelled\n• Keys deleted\n• Bot locked\n\nTo unlock: /reset',
    kill_switch_cancelled: '❌ Kill Switch cancelled.',
  }
};
...
// ============================================================
// 10. TELEGRAM ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ
// ============================================================
...
function generateCSV(riskReport) {
  ...
  if (riskReport.sharpe !== undefined) {
    ...
    csv += `Sortino Ratio,${riskReport.sortino.toFixed(2)}\n`;
    csv += `VaR (95%),${riskReport.var.toFixed(2)}%\n`;
  }
  return csv;
}
...
// ============================================================
// 13. РАСШИРЕННЫЙ АНАЛИЗ ПОРТФЕЛЯ (С КЭШЕМ, ПАРАЛЛЕЛИЗМОМ, НОВЫМИ МЕТРИКАМИ)
// ============================================================
const CACHE = new Map();
const CACHE_TTL = 60000; // 1 минута
const COMMISSION_RATE = 0.001; // 0.1%

// Кэширование тикеров и OHLCV
...
// Расширенные метрики: Sharpe, Sortino, VaR
...
async function calculateVaR(exchange, symbol, confidence = 0.95, days = 30) {
  ...
  return returns[index] * 100; // в процентах
}
...
// Основная функция анализа портфеля (обновлённая)
async function analyzePortfolio(exchangeId, apiKey, secretKey, chatId = null, lang = 'ru') {
  ...
  // Анализ рисков и рекомендации (улучшенный)
  const riskReport = await analyzeRisksEnhanced(totalUSDT, btcPercent, altPercent, usdtPercent, altcoins, btcMetrics, assetMetrics, exchange);

  // Сохранение истории портфеля (90 дней)
  if (chatId) {
    ...
    const maxEntries = CONFIG.PORTFOLIO_HISTORY_DAYS;
...
// Улучшенный анализ рисков (с новыми метриками)
async function analyzeRisksEnhanced(totalUSDT, btcPercent, altPercent, usdtPercent, altcoins, btcMetrics, assetMetrics, exchange) {
  const issues = [], recommendations = [], assets = [];
    ...
    if (currentPrice < ma200) {
      ...
      if (usdtPercent < targetStable) {
        ...
        for (const alt of sortedAlts) {
          if (remaining <= 0 || alt.price <= 0) break;
      ...
      if (btcPercent < 30 && btcMetrics.ma200.currentPrice > 0) {
        recommendations.push({
          id: `buy_btc_${Date.now()}`,
          action: 'buy',
          asset: 'BTC',
          symbol: 'BTC/USDT',
          amount: (0.01 * totalUSDT) / btcMetrics.ma200.currentPrice,
          target: 30,
  ...
  // 7. Новые метрики (Sharpe, Sortino, VaR)
  if (btcMetrics.sharpe < 0.5) {
    issues.push({
      type: 'low_sharpe',
      severity: 5,
      description: `Коэффициент Шарпа BTC: ${btcMetrics.sharpe.toFixed(2)} – низкая эффективность риска.`
    });
  }
  if (btcMetrics.var && btcMetrics.var < -5) {
    issues.push({
      type: 'high_var',
      severity: 6,
      description: `VaR (95%) BTC: ${btcMetrics.var.toFixed(2)}% – высокий риск падения.`
    });
  }
  for (const m of assetMetrics) {
    if (m.correlation && m.correlation > 0.8) {
      issues.push({
        ...
        severity: 4,
        description: `${m.asset} сильно коррелирует с BTC (${m.correlation.toFixed(2)}), снижает диверсификацию.`
      });
    }
    if (m.sharpe < 0.3) {
      issues.push({
        type: 'low_sharpe_asset',
        severity: 3,
        description: `${m.asset}: коэффициент Шарпа ${m.sharpe.toFixed(2)} – неэффективное использование риска.`
      });
    }
  }
  ...
  if (highSeverityCount > 2 || issues.some(i => i.type === 'delisting')) riskLevel = 'high';
  else if (highSeverityCount > 0 || mediumSeverityCount > 3) riskLevel = 'medium';
  else riskLevel = 'low';
...
// ============================================================
// 14. ИСПОЛНЕНИЕ РЕКОМЕНДАЦИЙ (С ПОДТВЕРЖДЕНИЕМ)
// ============================================================
...
async function checkContract(address) {
  ...
  // Используем Etherscan для проверки верификации
  if (ETHERSCAN_API_KEY) {
    try {
      ...
      if (data.status === '1' && data.result) {
        riskScore = Math.max(0, riskScore - 30); // верифицирован – снижаем риск
      } else {
        riskScore += 30; // не верифицирован
      }
    } catch (e) {}
  }
...
// ============================================================
// 16. СОЦИАЛЬНЫЕ ТРЕНДЫ (С ВИРАЛЬНОСТЬЮ И ПРОГНОЗОМ)
// ============================================================
...
async function getCoinGeckoData(coinId) {
  ...
  try {
    ...
    const data = await response.json();
    if (!COINGECKO_CACHE) global.COINGECKO_CACHE = new Map();
...
// Глобальный кэш для CoinGecko
if (!global.COINGECKO_CACHE) global.COINGECKO_CACHE = new Map();
...
// ============================================================
// 17. НОВОСТИ (АГРЕГАЦИЯ ИЗ 3+ ИСТОЧНИКОВ)
// ============================================================
...
// ============================================================
// 19. AI-СОВЕТНИК (С ИСТОРИЕЙ ДИАЛОГОВ И ПОЯСНЕНИЯМИ)
// ============================================================
async function getPortfolioContext(chatId) {
  try {
    ...
    context += `📊 Распределение: BTC ${parsed.btcPercent?.toFixed(1) || 0}%, Альты ${parsed.altPercent?.toFixed(1) || 0}%, Стейблы ${parsed.usdtPercent?.toFixed(1) || 0}%\n`;
    if (parsed.assets) {
      context += `📈 Активы:\n`;
      for (const a of parsed.assets.slice(0, 5)) {
        context += `• ${a.symbol}: ${a.weight?.toFixed(1) || 0}%\n`;
      }
    }
    if (parsed.btcMetrics) {
      context += `📊 Метрики BTC: Sharpe ${parsed.btcMetrics.sharpe?.toFixed(2) || 0}, VaR ${parsed.btcMetrics.var?.toFixed(2) || 0}%\n`;
    }
    if (parsed.riskLevel) {
      const riskEmoji = parsed.riskLevel === 'high' ? '🔴' : parsed.riskLevel === 'medium' ? '🟡' : '🟢';
      context += `${riskEmoji} Уровень риска: ${parsed.riskLevel === 'high' ? 'Высокий' : parsed.riskLevel === 'medium' ? 'Средний' : 'Низкий'}\n`;
    }
    if (parsed.issues && parsed.issues.length > 0) {
      context += `⚠️ Проблемы: ${parsed.issues.slice(0, 2).map(i => i.description).join('; ')}\n`;
    }
    return context;
  } catch (e) { return null; }
}
...
async function getSmartHelpResponse(chatId, question, lang) {
  const historyKey = `ai_history_${chatId}`;
  let history = await VOID_KV.get(historyKey);
  history = history ? JSON.parse(history) : [];
  history.push({ role: 'user', content: question });
  if (history.length > 10) history = history.slice(-10);
  const context = await getPortfolioContext(chatId);
  const isRu = lang === 'ru';
  const systemPrompt = `Ты – AI-крипто-советник. Данные портфеля: ${context || 'нет данных'}. Отвечай на вопросы, давай конкретные рекомендации и обязательно объясняй последствия каждого совета: что произойдёт, если выполнить ордер, финансовые последствия, риски, альтернативы. Язык: ${isRu ? 'русский' : 'английский'}.`;
  try {
...
async function runAutotrade() {
  ...
  for (const key of keys) {
    ...
    if (config.level === 1) {
      // Уровень 1: Защита – установка стоп-лоссов при падении 5% и 10%
      for (const coin of coins) {
        try {
          ...
          // Проверяем, есть ли уже стоп-лосс (упрощённо – создаём)
      ...
      // Уровень 2: Перераспределение – продажа мусорных токенов, покупка растущих
      for (const coin of coins) {
        try {
          const ticker = await exchange.fetchTicker(`${coin}/USDT`);
      ...
      // Уровень 3: Умный рост – трейлинг стоп-лосс и фиксация прибыли
...
async function createAlert(chatId, type, params) {
  ...
  if (alerts.length >= limit && limit !== Infinity) return { error: `Лимит оповещений: ${limit}` };
  const alert = { id: Date.now().toString(), type, params, createdAt: Date.now() };
  alerts.push(alert);
    ...
    for (const alert of alerts) {
      try {
        ...
        if (alert.type === 'price') {
          ...
          // Здесь можно реализовать проверку изменения за час, но для простоты удалим
...
// ============================================================
// 22. ХОЛОДНЫЙ ДУШ (АВТОМАТИЧЕСКАЯ ПРОВЕРКА BTC)
// ============================================================
...
async function composeRiskMessage(riskReport, mode, lang, dailyChange = 0) {
  ...
  if (sharpe !== undefined) {
    ...
    if (sortino !== undefined) {
      const sortinoEmoji = sortino > 1 ? '🟢' : (sortino > 0.5 ? '🟡' : '🔴');
      baseText += `${sortinoEmoji} Коэф. Сортино: ${sortino.toFixed(2)}\n`;
    }
    if (var_ !== undefined) {
      const varEmoji = var_ > -5 ? '🟢' : (var_ > -10 ? '🟡' : '🔴');
      baseText += `${varEmoji} VaR (95%): ${var_.toFixed(2)}%\n`;
    }
    baseText += `\n`;
  }
...
async function handleCallback(update) {
  ...
  try {
    ...
    if (data.startsWith('panic_convert_')) {
      const keysUser = await loadUserKeys(chatId);
      if (!keysUser) { await sendMessage(chatId, '❌ Нет ключей'); return; }
      const exchange = await getExchange(keysUser.exchangeId, keysUser.apiKey, keysUser.secretKey);
      const balance = await exchange.fetchBalance();
      const coins = Object.keys(balance.total).filter(c => c !== 'USDT' && balance.total[c] > 0);
      for (const coin of coins) {
        try {
          await exchange.createMarketSellOrder(`${coin}/USDT`, balance.total[coin]);
        } catch (e) {}
      }
      await sendMessage(chatId, getText(lang, 'panic_converted'));
      await VOID_KV.delete(`panic_${chatId}`);
      return;
    }
...
// ============================================================
// 26. ОБРАБОТЧИК СООБЩЕНИЙ (ПОЛНЫЙ)
// ============================================================
  ...
  try {
    ...
    if (state === 'alert_price') {
      ...
      if (parts.length === 2) {
        ...
        if (!isNaN(target)) {
          await createAlert(chatId, 'price', { symbol, target, direction: 'above' });
        ...
        const target = parseFloat(parts[1]);
        if (!isNaN(target)) {
          await createAlert(chatId, 'price', { symbol, target, direction: 'below' });
          await sendMessage(chatId, getText(lang, 'alert_created'));
          await setUserState(chatId, 'idle');
          return;
        }
    ...
    if (state === 'alert_change') {
      ...
      if (parts.length === 2) {
        ...
        const target = parseFloat(parts[1]);
        if (!isNaN(target)) {
          await createAlert(chatId, 'change', { symbol, target });
          await sendMessage(chatId, getText(lang, 'alert_created'));
          await setUserState(chatId, 'idle');
          return;
        }
    ...
    if (state === 'alert_volume') {
      ...
      if (parts.length === 2) {
        ...
        const target = parseFloat(parts[1]);
        if (!isNaN(target)) {
          await createAlert(chatId, 'volume', { symbol, target });
          await sendMessage(chatId, getText(lang, 'alert_created'));
          await setUserState(chatId, 'idle');
          return;
        }
    ...
    if (state === 'alert_news' || state === 'alert_calendar') {
      // Для новостей и календаря просто создаём оповещение без параметров
      const type = state === 'alert_news' ? 'news' : 'calendar';
      await createAlert(chatId, type, {});
      await sendMessage(chatId, getText(lang, 'alert_created'));
      await setUserState(chatId, 'idle');
      return;
    }
...
// ============================================================
// 27. ОБРАБОТЧИКИ АНТИСКАМА (ВСПОМОГАТЕЛЬНЫЕ)
// ============================================================
async function handleAntiScamInput(chatId, text, lang, update, messageId) {
  ...
  switch (state) {
    ...
    case 'antiscam_dex':
      ...
      // Используем отдельный лимит dex
...
// Вспомогательные функции для антискама
...
// ============================================================
// 27.5. ОБРАБОТЧИКИ ТРЕНДОВ И НОВОСТЕЙ (ВСПОМОГАТЕЛЬНЫЕ)
// ============================================================
...
// ============================================================
// 27.6. ОБРАБОТЧИКИ НОВОСТЕЙ И КАЛЕНДАРЯ
// ============================================================
  ...
  for (const query of queries) {
    try {
      ...
      const params = new URLSearchParams({
        ...
        language: isRu ? 'ru' : 'en',
        apiKey: NEWS_API_KEY,
        domains: isRu ? 'cointelegraph.com,news.bitcoin.com,cryptopotato.com,beincrypto.com,coindesk.com' : 'cointelegraph.com,news.bitcoin.com,cryptopotato.com,beincrypto.com,coindesk.com'
      });
...
async function handleCalendarCommand(chatId, lang, messageId) {
  const plan = await getUserPlan(chatId);
  if (!plan.limits.panic) { // используем panic как индикатор доступа к PRO/VIP
...
// ============================================================
// 29. ПЛАНЫ (ОБРАБОТЧИК ВЫБОРА ТАРИФА)
// ============================================================
