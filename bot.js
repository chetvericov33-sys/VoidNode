// ============================================================
// БОТ VOID NODE — ПОЛНАЯ ВЕРСИЯ 5.0 (ЧАСТЬ 1/7)
// ============================================================
...
// ============================================================
// 10. БЕЗОПАСНОСТЬ — ВАЛИДАЦИЯ API-КЛЮЧЕЙ
// ============================================================
    ...
    try {
        ...
        try {
            ...
            return {
                ...
                warning: '⚠️ Ваш API-ключ имеет права на вывод средств. Для безопасности рекомендуется использовать ключ только с правами на чтение.'
            };
        } catch (e) {
            return {
                valid: true,
...
// ============================================================
// 11. БЕЗОПАСНОСТЬ — ОГРАНИЧЕНИЕ ПОПЫТОК ПОДКЛЮЧЕНИЯ
// ============================================================
...
// ============================================================
// 12. БЕЗОПАСНОСТЬ — САНИТИЗАЦИЯ ВВОДА
// ============================================================
...
// ============================================================
// 14. БЕЗОПАСНОСТЬ — ВЕРИФИКАЦИЯ WEBHOOK CRYPTOBOT
// ============================================================
...
// ============================================================
// 15. БЕЗОПАСНОСТЬ — ЗАЩИТА ОТ ДВОЙНОЙ ОПЛАТЫ
// ============================================================
...
// ============================================================
// 16. БЕЗОПАСНОСТЬ — ПРОВЕРКА ПОДОЗРИТЕЛЬНЫХ АККАУНТОВ
// ============================================================
async function checkSuspiciousUser(chatId) {
    ...
    if (accountAge && Date.now() - parseInt(accountAge) < 7 * 24 * 60 * 60 * 1000) {
        return {
            suspicious: true,
            message: '⚠️ Ваш аккаунт был создан недавно. Будьте осторожны с подключением API-ключей.'
        };
    }
...
// ============================================================
// 18. КНОПКА ВЫХОДА (ДОБАВЛЯЕТСЯ В КАЖДОЕ СООБЩЕНИЕ)
// ============================================================
function addExitButton(keyboard, lang) {
    ...
    const hasExit = keyboard.inline_keyboard.some(row =>
        row.some(btn => btn.callback_data === 'exit_to_menu')
    );
...
// ============================================================
// 20. TELEGRAM ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ
// ============================================================
...
// ============================================================
// 21. АВТОМАТИЧЕСКАЯ ОЧИСТКА СТАРЫХ ДАННЫХ
// ============================================================
...
// ============================================================
// БОТ VOID NODE — ПОЛНАЯ ВЕРСИЯ 5.0 (ЧАСТЬ 2/7)
// ============================================================

// ============================================================
// 22. ТАРИФЫ (PLANS) — ПОЛНЫЙ ОБЪЕКТ
// ============================================================
const PLANS = {
    TRIAL: {
        id: 'TRIAL',
...
// ============================================================
// 23. МУЛЬТИЯЗЫЧНОСТЬ (LANGUAGES) — ПОЛНЫЙ ОБЪЕКТ
// ============================================================
const LANGUAGES = {
    ru: {
        // === ОБЩИЕ ===
        ...
        autotrade_active: (level) => `✅ Автоторговля активирована (уровень ${level})`,
        ...
        // === ДОБАВЛЯЕМ VIP-БОНУС И УВЕДОМЛЕНИЯ ===
        ...
        vip_bonus_skip_btn: '⏭️ Пропустить',
    ...
    en: {
        ...
        autotrade_active: (level) => `✅ Autotrading activated (level ${level})`,
        ...
        // VIP Bonus and Reminders (English)
...
async function getUserPlan(chatId) {
    ...
    if (!data) {
        ...
        if (!plan) {
            ...
            // Проверяем триал
            if (parsed.planId === 'TRIAL' && parsed.trialUsed && parsed.expires < Date.now()) {
                result = {
                    plan: 'NONE',
                    name: '❌ Без подписки',
                ...
                // Проверяем, не истёк ли VIP-триал
                if (parsed.vipTrial) {
                    // Отправляем уведомление об истечении VIP
                    const notifiedKey = `vip_expired_notified_${chatId}`;
                    const alreadyNotified = await getData(notifiedKey);
                    if (!alreadyNotified) {
                        const lang = await getData(`lang_${chatId}`) || 'ru';
                        await sendMessage(chatId,
                            `💔 VIP-доступ закончился. Было круто! Надеюсь, тебе понравилось.\n\n💫 Но это не прощание! Ты всегда можешь вернуться: /subscribe\n🛡️ Я рядом, если что: /help`
                        );
...
// ============================================================
// БОТ VOID NODE — ПОЛНАЯ ВЕРСИЯ 5.0 (ЧАСТЬ 3/7)
// ============================================================
...
// ============================================================
// 33. АВТОМАТИЧЕСКАЯ ПРОВЕРКА ССЫЛОК И КОНТРАКТОВ
// ============================================================
...
// ============================================================
// 35. СОЦ.ТРЕНДЫ (С КЭШЕМ COINGECKO)
// ============================================================
...
// ============================================================
// 36. НОВОСТИ (ПЕРСОНАЛИЗИРОВАННЫЕ С КЭШЕМ)
// ============================================================
class NewsManager {
    constructor() {
    ...
    async getPersonalizedNews(chatId, lang, forceUpdate = false) {
    ...
    async backgroundUpdate() {
        const keys = await VOID_KV.list('news_cache_');
        const now = Date.now();

        for (const key of keys.keys) {
            const chatId = parseInt(key.name.replace('news_cache_', ''));
            const data = await getData(key.name);
            if (!data) continue;
    ...
    async refreshNews(chatId, lang) {
        const result = await this._fetchAndCacheNews(chatId, lang, false);
        this.lastUpdate.set(chatId, Date.now());
        return result;
    }

    async _fetchAndCacheNews(chatId, lang, silent = false) {
        const isRu = lang === 'ru';
        ...
        if (assets.length === 0) {
            const defaultAssets = ['BTC', 'ETH', 'SOL', 'BNB', 'ADA'];
            assets = defaultAssets.map(symbol => ({ symbol, weight: 20 }));
        }
        ...
        const newsPromises = topAssets.map(async (asset) => {
            try {
    ...
    getStats(chatId) {
        const lastUpdate = this.lastUpdate.get(chatId);
        const isUpdating = this.isUpdating.get(chatId) || false;
        return {
...
async function handleNewsCommand(chatId, coin, lang, messageId) {
    ...
    if (result.fromCache) {
        report += `🕐 *${isRu ? `Обновлено ${result.age} мин. назад` : `Updated ${result.age} min ago`}*\n`;
    } else {
        report += `🕐 *${isRu ? 'Только что обновлено' : 'Just updated'}* 🔄\n`;
    }

    report += `📌 ${isRu ? `По твоим активам: ${result.assets.map(a => a.symbol).join(', ')}` : `For your assets: ${result.assets.map(a => a.symbol).join(', ')}`}\n`;
    report += `━━━━━━━━━━━━━━━━━━━━━━━\n\n`;

    if (result.articles.length === 0) {
        report += `📭 ${isRu ? 'Новостей не найдено' : 'No news found'}\n`;
        report += `🔄 ${isRu ? 'Попробуй обновить через 5-10 минут' : 'Try refreshing in 5-10 minutes'}`;
    ...
    for (const article of result.articles.slice(0, 7)) {
    ...
    report += `━━━━━━━━━━━━━━━━━━━━━━━\n`;
    report += `📊 ${isRu ? `Найдено: ${result.articles.length} новостей` : `Found: ${result.articles.length} news`}\n`;
...
// ============================================================
// БОТ VOID NODE — ПОЛНАЯ ВЕРСИЯ 5.0 (ЧАСТЬ 4/7)
// ============================================================
...
async function showMainMenu(chatId, lang) {
    ...
    let vipStatus = '';
    const isVip = userPlan.plan === 'VIP' && userPlan.expires > Date.now();
    if (isVip) {
...
async function showAboutMenu(chatId, lang) {
    ...
    message += `🛡️ *Void Node* — твой персональный крипто-телохранитель.\n\n`;
    message += `🤖 *Что умеет бот:*\n`;
    message += `• 📊 Анализировать портфель и давать рекомендации\n`;
    message += `• 🛡️ Проверять контракты и ссылки на безопасность\n`;
    ...
    message += `• ❄️ Холодный душ при падении рынка\n`;
    message += `• 🔔 Оповещения по цене, объёму и новостям\n\n`;
    message += `📌 *Версия:* 2.0.0\n`;
    message += `📅 *Создан:* 2024\n\n`;
    message += `━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
    message += `📢 *Канал проекта:*\n`;
    message += `[Atifragility Node](https://t.me/atifragility_node)\n\n`;
    message += `━━━━━━━━━━━━━━━━━━━━━━━\n`;
...
async function showPlansMenu(chatId, lang) {
    ...
    message += `━━━━━━━━━━━━━━━━━━━━━━━\n`;
    message += `💳 ${isRu ? 'Оплата через CryptoBot (USDT)' : 'Payment via CryptoBot (USDT)'}\n`;
...
async function handlePlanSelection(chatId, planId, lang, messageId) {
    ...
    message += displayFeatures.map(f => `• ${f}`).join('\n');
    message += `\n\n━━━━━━━━━━━━━━━━━━━━━━━\n`;
    ...
    const keyboard = {
        inline_keyboard: [
            [{ text: `💳 ${isRu ? 'Оплатить' : 'Pay'} ${plan.price} ₽ (USDT)`, url: invoice.payUrl }],
...
// ============================================================
// БОТ VOID NODE — ПОЛНАЯ ВЕРСИЯ 5.0 (ЧАСТЬ 5/7)
// ============================================================

// ============================================================
// 42. СТРАТЕГИЯ "СНЕЖНЫЙ КОМ" — ТРЕКЕР
// ============================================================
...
// ============================================================
// 43. СТРАТЕГИЯ "СНЕЖНЫЙ КОМ" — ОСНОВНАЯ ЛОГИКА
// ============================================================
...
async function runSnowballStrategy(chatId) {
    ...
    try {
        ...
        for (const [symbol, amount] of Object.entries(total)) {
            if (symbol === 'USDT' || amount < 0.0001) continue;
            try {
        ...
        if (junkToken.change24h > 0) {
            const nonGrowing = tokens.filter(t => t.change24h < 2);
        ...
        if (growth >= 30) {
            ...
            if (tokenBalance > 0.001) {
                ...
                const message = `❄️ *ПРИБЫЛЬ ЗАФИКСИРОВАНА!*\n\n` +
...
// ============================================================
// 44. ХОЛОДНЫЙ ДУШ — ПРОВЕРКА ВСЕХ ТОКЕНОВ
// ============================================================
        ...
        try {
            ...
            for (const coin of coins) {
                try {
                    ...
                    let previousPrice = await getData(priceKey);
                    ...
                    if (previousPrice) {
                        ...
                        if (drop >= 5) {
                            panicTriggered = true;
                            droppedCoins.push({ coin, drop, currentPrice, prev });
                        }
                    }
            ...
            if (panicTriggered) {
                const coinsList = droppedCoins.map(c => `• ${c.coin}: -${c.drop.toFixed(1)}%`).join('\n');
                
                const message = `🚨 *ХОЛОДНЫЙ ДУШ СРАБОТАЛ!*\n━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
                    `⚠️ Обнаружено падение >5% по нескольким активам:\n\n${coinsList}\n\n` +
                    `🛡️ *Рекомендация:* конвертировать ВСЕ активы в USDT для защиты капитала.`;
...
async function handlePanicConvertAll(chatId) {
    ...
    try {
        ...
        for (const k of priceKeys.keys) {
            await deleteData(k.name);
        }
...
// ============================================================
// 45. VIP-УВЕДОМЛЕНИЯ (ОДНО ЗА 24 ЧАСА)
// ============================================================
    ...
    async checkAllUsers() {
        ...
        for (const key of keys.keys) {
            ...
            try {
                ...
                const timeLeft = plan.expires - now;
                const lang = await getData(`lang_${chatId}`) || 'ru';
                const isRu = lang === 'ru';
                
                const notifiedKey = `vip_notified_${chatId}`;
                const alreadyNotified = await getData(notifiedKey);
                ...
                if (timeLeft <= 24 * 60 * 60 * 1000 && 
                    ...
                    const expiresDate = formatDateShort(plan.expires);
                    const message = isRu
                ...
                console.error(`❌ Ошибка проверки VIP для ${chatId}:`, error);
            }
        }
    }
}

const vipReminder = new VipReminderService();

// ============================================================
// 46. ЗАГРУЗКА КЛЮЧЕЙ (ВСПОМОГАТЕЛЬНАЯ)
// ============================================================
...
// ============================================================
// 49. АВТОМАТИЧЕСКАЯ ТОРГОВЛЯ (ФОНОВАЯ)
// ============================================================
...
async function runAutotrade() {
    ...
    for (const key of keys.keys) {
        ...
        try {
            ...
            if (config.level === 4) {
                console.log(`❄️ Запуск "Снежного кома" для ${chatId}`);
...
// ============================================================
// БОТ VOID NODE — ПОЛНАЯ ВЕРСИЯ 5.0 (ЧАСТЬ 6/7)
// ============================================================

// ============================================================
// 51. AI-ИНТЕГРАЦИЯ (ПОДКЛЮЧЕНИЕ ai_router.js)
// ============================================================

const aiRouter = require('./ai_router');

// ============================================================
// 52. ОБРАБОТЧИК CALLBACK (ВСЕ КНОПКИ)
// ============================================================
...
async function handleCallback(update) {
    ...
    try {
        ...
        if (data === 'autotrade_level4') {
            ...
            await sendUpdatedMessage(chatId, '❄️ Запускаю "Снежный ком"...', null, 'Markdown');
...
async function handleMessage(update) {
    ...
    try {
        ...
        // Проверка, что сообщение от человека
        ...
        if (state === 'waiting_for_keys' || state === 'waiting_for_keys_vip') {
            ...
            if (parts.length === 2) {
                ...
                if (state === 'waiting_for_keys_vip') {
                    ...
                    const message = `👑 *${isRu ? 'ПОЗДРАВЛЯЮ!' : 'CONGRATULATIONS!'}*\n━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
                        `🎉 ${isRu ? 'Ты подключил биржу и получил VIP-доступ на 3 дня!' : 'You connected your exchange and got 3 days of VIP access!'}\n\n` +
                        `📅 ${isRu ? 'Действует до:' : 'Valid until:'} ${expiresDate}\n\n` +
                        `✨ ${isRu ? 'Теперь тебе доступно всё:' : 'Now you have access to everything:'}\n` +
                        `• 🛡️ ${isRu ? 'Холодный душ' : 'Panic mode'}\n` +
                        `• 🚀 ${isRu ? 'Автоторговля' : 'Autotrading'}\n` +
                        `• 📊 ${isRu ? 'Полная аналитика' : 'Full analytics'}\n` +
                        `• 🔔 ${isRu ? 'Безлимитные оповещения' : 'Unlimited alerts'}\n\n` +
                        `💡 ${isRu ? 'Попробуй все функции, а после триала выбери подходящий тариф!' : 'Try all features, then choose a plan that suits you!'}`;
                    const keyboard = {
                        inline_keyboard: [
                            [{ text: `🏠 ${isRu ? 'В меню' : 'To menu'}`, callback_data: 'onboard_vip_done' }]
                        ]
                    };
                    await sendUpdatedMessage(chatId, message, keyboard, 'Markdown', messageId);
                    return;
        ...
        if (state === 'waiting_for_contract_search') {
            ...
            const cleanInput = cleanText.trim();
            ...
            if (parts.length === 2) {
                ...
                if (!isNaN(target)) {
                    ...
                    const alert = { 
                        ...
                        type: 'price', 
                        params: { symbol, target, direction: 'above' }, 
                        active: true, 
                        createdAt: Date.now() 
                    };
        ...
        if (state === 'ai_chat') {
            ...
            await sendTyping(chatId);
            const portfolioData = await getData(`analysis_${chatId}`);
            const portfolio = portfolioData ? JSON.parse(portfolioData) : null;
            const answer = await aiRouter.safeAIChat(cleanText, portfolio, lang);
            if (answer) {
                ...
                for (const coin of coins) {
                    if (coin === 'USDT') {
                        ...
                        try {
                            ...
                            const value = total[coin] * ticker.last;
                            totalUSDT += value;
                            assets.push({ symbol: coin, value, amount: total[coin] });
                        } catch (e) {
        ...
        if (cleanText === '/analyze') {
            ...
            // ... (полный код анализа, такой же как в action_analyze)
            return;
        }
...
// ============================================================
// БОТ VOID NODE — ПОЛНАЯ ВЕРСИЯ 5.0 (ЧАСТЬ 7/7 — ФИНАЛ)
// ============================================================
...
// Запуск задачи с восстановлением при ошибках
function runTaskWithRecovery(task, name, interval) {
    ...
    // Первый запуск через 5 секунд после старта бота
    setTimeout(run, 5000);
}

// 55.1. Проверка оповещений
async function checkAlerts() {
    ...
    for (const key of keys.keys) {
        ...
        const keysUser = await loadUserKeys(chatId);
        if (!keysUser) {
            console.warn(`⚠️ Нет ключей для ${chatId}, пропускаем оповещения`);
            continue;
        }
            ...
            for (const alert of parsedAlerts) {
                ...
                try {
                    ...
                    if (alert.type === 'price') {
                        ...
                        // Проверяем изменение за час (нужно хранить предыдущую цену)
...
// 55.2. Проверка новостей (фоновое обновление)
async function runNewsBackgroundUpdate() {
    await newsManager.backgroundUpdate();
}

// 55.3. Ежедневная проверка истекающих подписок
async function checkExpiringPlans() {
    ...
    for (const key of keys.keys) {
        ...
        // Проверяем только платные подписки (не TRIAL и не VIP-триал)
        ...
        if (timeLeft < threeDays && timeLeft > 0) {
            ...
            const notifiedKey = `expire_notified_${chatId}`;
            const alreadyNotified = await getData(notifiedKey);
            if (!alreadyNotified) {
                await sendMessage(chatId,
                    `⚠️ *${isRu ? 'Твоя подписка заканчивается через 3 дня!' : 'Your subscription expires in 3 days!'}*\n\n` +
                    `📅 ${isRu ? 'Дата окончания:' : 'Expires:'} ${expiresDate}\n\n` +
                    `💳 ${isRu ? 'Чтобы продлить — отправь /subscribe' : 'To renew — send /subscribe'}`,
                    null, 'Markdown'
                );
...
// 55.4. Очистка кэша планов (каждые 10 минут)
async function cleanPlanCache() {
    const now = Date.now();
    for (const [chatId, entry] of planCache) {
        if (now - entry.timestamp > 600000) { // 10 минут
            planCache.delete(chatId);
        }
    }
}

// 55.5. Проверка VIP-уведомлений (за 24 часа)
async function runVipReminderCheck() {
    await vipReminder.checkAllUsers();
}

// ============================================================
// 56. ЗАПУСК ФОНОВЫХ ЗАДАЧ
// ============================================================

// Оповещения — каждые 5 минут
runTaskWithRecovery(checkAlerts, 'checkAlerts', CONFIG.ALERT_CHECK_INTERVAL);

// Автоторговля — каждые 15 минут
runTaskWithRecovery(runAutotrade, 'runAutotrade', CONFIG.AUTOTRADE_CHECK_INTERVAL);

// Холодный душ — каждые 15 минут
...
// Очистка кэша планов — каждые 10 минут
...
// Webhook для отладки (можно удалить)
...
// Запускаем сервер на всех интерфейсах (для Render)
...
// ============================================================
// 59. ОБРАБОТКА КРИТИЧЕСКИХ ОШИБОК
// ============================================================
...
process.on('uncaughtException', (error) => {
    ...
    // Не закрываем процесс — даём боту шанс восстановиться
});
...
// ============================================================
// 60. ПЕРВОНАЧАЛЬНЫЙ ЗАПУСК ЗАДАЧ (при старте)
// ============================================================

// Запускаем задачи при старте с небольшой задержкой
setTimeout(async () => {
    try {
...
// ============================================================
// КОНЕЦ ФАЙЛА — БОТ ПОЛНОСТЬЮ ЗАГРУЖЕН
// ============================================================

console.log('━━━━━━━━━━━━━━━━━━━━━━━');
console.log('🚀 БОТ VOID NODE 5.0 УСПЕШНО ЗАПУЩЕН!');
